...
class privateChannels(commands.Cog):
    ...
    def embed_privateChannelMessage():
        ...
        embed.add_field(
            name="Si tu souhaites témoigner mais que tu veux rester anonyme, si tu as besoin de nous contacter ou que tu recherches simplement un espace pour pouvoir vider ton sac, tu peux créer un canal privé !",
            value="",
            inline=False,
        )
        embed.set_footer(
            text="Ce qui est dit dans les canaux privés n'est pas enregistré, sauf si tu donnes explicitement ton accord !"
        )
        return embed

    def embed_welcome():
        embed = discord.Embed(
            title="🔒 Ton canal privé 🔒",
            description="Exprime-toi comme tu l'entends ! Tu peux ici témoigner anonymement, ou bien nous contacter si tu as besoin d'aide !",
            color=0x09B6E1,
        )
        embed.add_field(
            name="🔗 Partager anonymement ton témoignage",
            value='> Ton témoignage peut apparaître dans le canal "réponse anonyme" du jour que tu souhaites ! Pour ce faire, clique simplement sur le jour auquel ton témoignage fait référence et suis les instructions.',
            inline=False,
        )
        embed.set_footer(
            text="Le contenu de ce canal ne sera pas retenu dans la synthèse."
        )
        return embed

    def embed_share(day: int):
        embed = discord.Embed(
            title="▶️ Partager ton témoignage", color=0x1AE843
        )
        embed.add_field(
            name="",
            value=f"> Réagis avec '✅' aux messages que tu souhaites partager \n\n > Clique sur 'Confirmer le partage' et ton message sera partagé dans le canal de réponses anonymes du **Jour {day}** !",
            inline=False,
        )
        return embed
    ...
    def embed_close():
        embed = discord.Embed(
            title="❌ Confirmer la suppression ❌ ", color=0xFF0000
        )
        embed.add_field(
            name="Es-tu sûre de vouloir supprimer le canal ?",
            value="Toute suppression est définitive et irréversible. Les messages seront perdus.",
            inline=False,
        )
        return embed
    ...
    class PrivateChannelButton(discord.ui.View):
        def __init__(self):
            super().__init__(
                timeout=None
            )  # Il est nécessaire d'explicitement définir le timeout à None pour permettre la persistance
    ...
    class ConfirmShareButton(discord.ui.View):
            ...
            async for message in history:
                if message.author == author:
                    for reaction in message.reactions:
                        if reaction.emoji == "✅" and author in [
        ...
        class ConfirmButton(discord.ui.Button):
            ...
            async def callback(self, interaction: discord.Interaction):
                ...
                if (
                    interaction.user.id
                    in privateChannels.ConfirmShareButton.slow_mode.keys()
                    and time.time()
                    <= privateChannels.ConfirmShareButton.slow_mode[
                        interaction.user.id
                    ]
                ):
                    await interaction.response.send_message(
                        "Tu ne peux faire qu'un seul partage toutes les 15 minutes. Merci de patienter avant de pouvoir faire le prochain !",
                        delete_after=30,
                    )
                    return

                message_list = (
                    await privateChannels.ConfirmShareButton.get_messages(
                        interaction.user,
                        interaction.channel.history(limit=100),
                    )
                )

                if len(message_list) == 0:
                    await interaction.response.send_message(
                        "Tu n'as pas sélectionné de message !", delete_after=15
                    )
                    return

                forum = interaction.client.get_cog("ForumManager").get_forum(
                    interaction.guild.id
                )
                config = forum.config
                ano_answers = config["GENERAL"]["CHANNELS"]["DAYS"][self.day][
                    "ANO_ANSWERS"
                ]
                ...
                embed = discord.Embed()
                embed.add_field(
                    name="📢  Nouveau témoignage anonyme",
                    value="",
                    inline=False,
                )
                ...
                await interaction.channel.send(
                    f"✅ Ton témoignage a été partagé, {interaction.user.mention} !",
                    delete_after=20,
                )
                privateChannels.ConfirmShareButton.slow_mode[
                    interaction.user.id
                    ...
                    time.time()
                    + config["PRIVATE_CHANNELS"]["SHARING_COOLDOWN"]
                )
    ...
    class ShareButtons(discord.ui.View):
        ...
        class ShareButton(discord.ui.Button):
            ...
            async def callback(self, interaction: discord.Interaction):
                await interaction.channel.send(
                    embed=privateChannels.embed_share(int(self.custom_id) - 1),
                    view=privateChannels.ConfirmShareButton(
                        day=int(self.custom_id) - 2
                    ),
                    delete_after=900,
                )
                await interaction.response.defer()
        ...
        class RefreshButton(discord.ui.Button):
            ...
            async def callback(self, interaction: discord.Interaction):
                forum = self.bot.get_cog("ForumManager").get_forum(
                    interaction.guild.id
                )  # WARNING: BROKEN
                config = forum.config
                current_day = config["GENERAL"]["CURRENT_DAY"]
                await interaction.message.edit(
                    view=privateChannels.ShareButtons(
                        bot=self.bot, current_day=current_day
                    )
                )
                await interaction.response.defer()
        ...
        class CloseButton(discord.ui.Button):
            ...
            async def callback(self, interaction: discord.Interaction):
                await interaction.response.send_message(
                    embed=privateChannels.embed_close(),
                    view=privateChannels.ConfirmCloseButton(),
                    delete_after=120,
                )
    ...
    async def createPrivateChannel(interaction: discord.Interaction):
        ...
        guild = (
            interaction.guild
        )  # La 'guild' est la dénomination de discordpy pour représenter un serveur discord
        user = (
            interaction.user
        )  # La personne qui demande la création d'un canal privé
        ...
        if (
            "channels" in data.keys()
            and str(user.id) in data["channels"].keys()
        ):
            channel = guild.get_channel(data["channels"][str(user.id)])
            await channel.send(
                content=f"⚠️ Tu ne peux posséder qu'un seul canal privé, {user.mention}.",
                delete_after=20,
            )
            return
        ...
        channel = await interaction.channel.category.create_text_channel(
            name="canal privé " + "{:04d}".format(data["counter"]),
            overwrites=overwrites,
        )
        await channel.send(
            embed=privateChannels.embed_welcome(),
            view=privateChannels.ShareButtons(
                bot=interaction.client, current_day=current_day
            ),
        )
        await channel.send(
            f"✅ Ton canal personnel a été créé {interaction.user.mention} !",
            delete_after=20,
        )
        ...
        # Ajout des canaux privés dans la liste des canaux à ne pas ajouter à la synthese
        not_log_channels = forum.get_data("do_not_log_channels")
        if type(not_log_channels) != list:
            not_log_channels = []
        for private_channel in data["channels"].values():
            not_log_channels.append(private_channel)
        forum.save_data("do_not_log_channels", list(set(not_log_channels)))
...
async def setup(bot) -> None:
    ...
    bot.add_view(
        privateChannels.PrivateChannelButton()
    )  # Persistance du bouton "Créer un canal privé"
    bot.add_view(
        privateChannels.ShareButtons(bot=bot)
    )  # Persistance des boutons de partage des jours
